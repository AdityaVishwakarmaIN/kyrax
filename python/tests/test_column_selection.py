...
@pytest.fixture
def excel_reader_single_sheet() -> kyrax.ExcelReader:
    return kyrax.read_excel(path_for_fixture("fixture-single-sheet.xlsx"))


@pytest.fixture
def expected_column_info() -> list[kyrax.ColumnInfo]:
    return [
        kyrax.ColumnInfo(
            name="Month",
            index=0,
...
def test_single_sheet_all_columns(
    excel_reader_single_sheet: kyrax.ExcelReader,
    expected_column_info: list[kyrax.ColumnInfo],
) -> None:
...
@pytest.fixture
def excel_reader_single_sheet_with_unnamed_columns() -> kyrax.ExcelReader:
    return kyrax.read_excel(path_for_fixture("fixture-multi-sheet.xlsx"))
...
@pytest.fixture
def sheet_with_unnamed_columns_expected_column_info() -> list[kyrax.ColumnInfo]:
    return [
        kyrax.ColumnInfo(
            name="col1",
            index=0,
...
def test_single_sheet_with_unnamed_columns(
    excel_reader_single_sheet_with_unnamed_columns: kyrax.ExcelReader,
    single_sheet_with_unnamed_columns_expected: dict[str, list[Any]],
    sheet_with_unnamed_columns_expected_column_info: list[kyrax.ColumnInfo],
) -> None:
...
"""
    with pytest.raises(kyrax.InvalidParametersError, match=re.escape(expected_message)):
...
"""
    with pytest.raises(kyrax.ColumnNotFoundError, match=expected_message):
...
def test_use_columns_with_column_names() -> None:
    excel_reader = kyrax.read_excel(path_for_fixture("fixture-single-sheet-with-types.xlsx"))
...
def test_use_columns_with_callable() -> None:
    excel_reader = kyrax.read_excel(path_for_fixture("fixture-multi-sheet.xlsx"))
...
def test_use_columns_with_bad_callable() -> None:
    ...
    with pytest.raises(
        kyrax.InvalidParametersError, match="`use_columns` callable should return a boolean"
    ):
        excel_reader.load_sheet(  # ty: ignore[no-matching-overload]
            2,
...
def test_use_columns_dtypes_eager_loading(
    ...
    for use_columns in (
        ...
        excel_reader = kyrax.read_excel(path_for_fixture(excel_file))
...
def test_use_columns_with_table() -> None:
    excel_reader = kyrax.read_excel(path_for_fixture("sheet-with-tables.xlsx"))
...
def test_use_column_range_with_offset_without_table() -> None:
    excel_reader = kyrax.read_excel(path_for_fixture("sheet-and-table-with-offset.xlsx"))
