...
def test_single_sheet_bytes():
    with open(path_for_fixture("fixture-single-sheet.xlsx"), "rb") as f:
        excel_reader = kyrax.read_excel(f.read())
    assert excel_reader.sheet_names == ["January"]
    sheet_by_name = excel_reader.load_sheet("January")
    sheet_by_idx = excel_reader.load_sheet(0)
...
def test_sheets_with_header_line_diff_from_zero():
    excel_reader = kyrax.read_excel(path_for_fixture("fixture-changing-header-location.xlsx"))
...
def test_sheet_with_pagination_out_of_bound():
    ...
    with pytest.raises(
        kyrax.InvalidParametersError, match="Too many rows skipped. Max height is 4"
    ):
...
def test_null_values_in_cells() -> None:
    excel_reader = kyrax.read_excel(path_for_fixture("fixture-invalid-cell-value.xlsx"))
    sheet = excel_reader.load_sheet(0)
...
def test_null_column_is_nullable() -> None:
    sheet = kyrax.read_excel(path_for_fixture("null-column.xlsx")).load_sheet(0)
    assert sheet.to_arrow().schema.field("nullonly").nullable is True


def test_sheet_with_decimal_numbers() -> None:
    sheet = kyrax.read_excel(path_for_fixture("decimal-numbers.xlsx")).load_sheet(0)
    pl_assert_frame_equal(
        sheet.to_polars(),
        pl.DataFrame({"Decimals": [28.14, 29.02]}),
    )

    sheet2 = kyrax.read_excel(path_for_fixture("decimal-numbers.xlsx")).load_sheet(
        0, dtypes={0: "string"}
    )
    pl_assert_frame_equal(
        sheet2.to_polars(),
...
def test_header_row_and_skip_rows(
    ...
    pl_assert_frame_equal(
        kyrax.read_excel(path_for_fixture("no-header.xlsx"))
...
def test_null_bytes_in_column_names() -> None:
    """upstream issue #343"""
    reader = kyrax.read_excel(path_for_fixture("null-bytes-in-columns-names.xls"))
