...
impl ExcelSheets {
    fn worksheet_range(&mut self, name: &str) -> KyraxResult<Range<Data>> {
        match self {
            Self::File(sheets) => sheets.worksheet_range(name),
            Self::Bytes(sheets) => sheets.worksheet_range(name),
        }
        .map_err(|err| KyraxErrorKind::CalamineError(err).into())
    ...
    fn table_names(&mut self, sheet_name: Option<&str>) -> KyraxResult<Vec<&str>> {
    ...
    fn defined_names(&mut self) -> KyraxResult<Vec<DefinedName>> {
    ...
    #[cfg(feature = "python")]
    fn worksheet_range_ref(&mut self, name: &str) -> KyraxResult<Range<DataRef<'_>>> {
        match self {
    ...
    fn get_table(&mut self, name: &str) -> KyraxResult<Table<Data>> {
        match self {
...
impl ExcelReader {
    ...
    pub(crate) fn try_from_path(path: &str) -> KyraxResult<Self> {
    ...
    fn find_sheet_meta(&self, idx_or_name: IdxOrName) -> KyraxResult<&CalamineSheet> {
        match idx_or_name {
            IdxOrName::Name(name) => {
                if let Some(sheet) = self.sheet_metadata.iter().find(|s| s.name == name) {
                    Ok(sheet)
                } else {
                    Err(KyraxErrorKind::SheetNotFound(IdxOrName::Name(name.clone())).into()).with_context(||  {
            ...
            IdxOrName::Idx(idx) => self
                .sheet_metadata
                .get(idx)
                .ok_or_else(|| KyraxErrorKind::SheetNotFound(IdxOrName::Idx(idx)).into())
                .with_context(|| {
                    format!(
    ...
    pub fn table_names(&mut self, sheet_name: Option<&str>) -> KyraxResult<Vec<&str>> {
        self.sheets.table_names(sheet_name)
    }

    pub fn defined_names(&mut self) -> KyraxResult<Vec<DefinedName>> {
