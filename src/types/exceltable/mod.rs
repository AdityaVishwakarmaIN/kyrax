...
impl ExcelTable {
    ...
    ) -> KyraxResult<(Vec<String>, Vec<IdxOrName>)> {
        ...
        let selected_column_indices = selected_columns
            ...
            .map(|idx_or_name| match idx_or_name {
                ...
                IdxOrName::Name(name) => table_columns
                    ...
                    .ok_or_else(|| KyraxErrorKind::ColumnNotFound(name.clone().into()).into())
                    .with_context(|| format!("available columns are: {table_columns:?}")),
            })
            .collect::<KyraxResult<Vec<usize>>>()?;
    ...
    fn build_header_and_update_selection(
        table: &Table<Data>,
        opts: LoadSheetOrTableOptions,
    ) -> KyraxResult<(Header, LoadSheetOrTableOptions)> {
    ...
    pub fn to_columns(&self) -> KyraxResult<Vec<KyraxColumn>> {
        self.selected_columns
            .iter()
            .map(|column_info| {
                KyraxColumn::try_from_column_info(
                    column_info,
                    self.table.data(),
                    self.offset(),
                    self.limit(),
    ...
    pub fn to_polars(&self) -> KyraxResult<DataFrame> {
        use crate::error::KyraxErrorKind;
