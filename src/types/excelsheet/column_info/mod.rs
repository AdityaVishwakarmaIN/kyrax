        ...
        Header::With(names) => {
            if let SelectedColumns::Selection(column_selection) = selected_columns {
                if column_selection.len() != names.len() {
                    return Err(KyraxErrorKind::InvalidParameters(
                ...
                let selected_indices = column_selection
                        ...
                        IdxOrName::Name(name) => Err(KyraxErrorKind::InvalidParameters(
...
impl AvailableColumns {
    pub(crate) fn as_loaded(&self) -> KyraxResult<&[ColumnInfo]> {
        match self {
            AvailableColumns::Loaded(column_infos) => Ok(column_infos),
            AvailableColumns::Pending => Err(KyraxErrorKind::Internal(format!(
                "Expected available columns to be loaded, got {self:?}. \
                    This is a bug, please report it to the kyrax repository"
            ))
            .into()),
        }
    }
