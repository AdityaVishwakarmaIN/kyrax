...
impl TryFrom<&Bound<'_, PyList>> for SelectedColumns {
    ...
    fn try_from(py_list: &Bound<'_, PyList>) -> KyraxResult<Self> {
...
impl TryFrom<Option<&Bound<'_, PyAny>>> for SelectedColumns {
    ...
    fn try_from(py_any_opt: Option<&Bound<'_, PyAny>>) -> KyraxResult<Self> {
        match py_any_opt {
            None => Ok(Self::All),
            Some(py_any) => {
...
impl SkipRows {
    pub(crate) fn should_skip_row(&self, row_idx: usize, py: Python) -> KyraxResult<bool> {
        match self {
...
impl<'a, 'py> FromPyObject<'a, 'py> for SkipRows {
    ...
    fn extract(obj: Borrowed<'a, 'py, PyAny>) -> Result<Self, Self::Error> {
        ...
        Err(KyraxErrorKind::InvalidParameters(
...
impl TryFrom<&ExcelSheet> for RecordBatch {
    ...
    fn try_from(sheet: &ExcelSheet) -> KyraxResult<Self> {
...
impl ExcelSheet {
    ...
    pub fn py_available_columns(&mut self) -> KyraxResult<Vec<ColumnInfo>> {
    ...
    #[getter("visible")]
    pub fn py_visible<'py>(&'py self, py: Python<'py>) -> KyraxResult<Bound<'py, PyString>> {
