...
use crate::{
    ...
    data::{ExcelSheetData, KyraxColumn},
    error::{ErrorContext, KyraxError, KyraxErrorKind, KyraxResult},
    ...
    pub(crate) fn try_new<CT: CellType>(
        ...
        if let SkipRows::Simple(skip_count) = &skip_rows {
            if max_height < *skip_count {
                return Err(KyraxErrorKind::InvalidParameters(format!(
...
impl SelectedColumns {
    ...
    ) -> KyraxResult<Vec<ColumnInfoNoDtype>> {
        match self {
            ...
            SelectedColumns::Selection(selection) => {
                let selected_indices: Vec<usize> = selection
                    ...
                    .map(|selected_column| {
                        ...
                        .ok_or_else(|| {
                            KyraxErrorKind::ColumnNotFound(selected_column.clone()).into()
                        })
                        .with_context(|| format!("available columns are: {available_columns:?}"))
                    })
                    .collect::<KyraxResult<_>>()?;
                ...
                available_columns
                    ...
                    .filter_map(
                        |col_info| match use_col_func.call1(py, (col_info.clone(),)) {
                            Err(err) => Some(Err(KyraxErrorKind::InvalidParameters(format!(
                                "`use_columns` callable could not be called ({err})"
                            ))
                            .into())),
                            Ok(should_use_col) => match should_use_col.extract::<bool>(py) {
                                Err(_) => Some(Err(KyraxErrorKind::InvalidParameters(
    ...
    fn col_idx_for_col_as_letter(col: &str) -> KyraxResult<usize> {
        use KyraxErrorKind::InvalidParameters;
        ...
        col.chars()
            ...
            .map(|(idx, col_chr)| {
                let pos_in_alphabet = Self::ALPHABET
                    ...
                    .ok_or_else(|| {
                        KyraxError::from(InvalidParameters(format!(
    ...
    fn col_indices_for_letter_range(col_range: &str) -> KyraxResult<Vec<usize>> {
        use KyraxErrorKind::InvalidParameters;
    ...
    fn col_selection_for_letter_range(
        col_range: &str,
    ) -> KyraxResult<Vec<DeferredColumnSelection>> {
        use KyraxErrorKind::InvalidParameters;
...
impl FromStr for SelectedColumns {
    ...
    fn from_str(s: &str) -> KyraxResult<Self> {
...
impl ExcelSheet {
    ...
    fn ensure_available_columns_loaded(&mut self) -> KyraxResult<()> {
    ...
    fn load_available_columns(&mut self) -> KyraxResult<&[ColumnInfo]> {
    ...
    pub fn available_columns(&mut self) -> KyraxResult<Vec<ColumnInfo>> {
    ...
    pub fn to_columns(&self) -> KyraxResult<Vec<KyraxColumn>> {
        self.selected_columns
            .iter()
            .map(|column_info| {
                ...
                match self.data() {
                    ExcelSheetData::Owned(range) => KyraxColumn::try_from_column_info(
                        column_info,
                        range,
                        offset,
                        limit,
                    ...
                    ExcelSheetData::Ref(range) => KyraxColumn::try_from_column_info(
                        column_info,
                        range,
                        offset,
                        limit,
...
mod tests {
    ...
    fn selected_columns_from_invalid_ints() {
        Python::attach(|py| {
            ...
            assert!(matches!(err.kind, KyraxErrorKind::InvalidParameters(_)));
        });
    }
    ...
    fn selected_columns_from_invalid_ranges(#[case] raw: &str, #[case] message: &str) {
        Python::attach(|py| {
            ...
            match err.kind {
                KyraxErrorKind::InvalidParameters(detail) => {
