...
use kyrax::{KyraxColumn, LoadSheetOrTableOptions, SkipRows};
#[cfg(feature = "polars")]
...
fn test_single_sheet() -> Result<()> {
    let mut reader = kyrax::read_excel(path_for_fixture("fixture-single-sheet.xlsx"))
...
fn test_single_sheet_bytes() -> Result<()> {
    ...
    let mut reader = kyrax::ExcelReader::try_from(bytes.as_slice())
...
fn test_single_sheet_with_types() -> Result<()> {
    let mut excel_reader =
        kyrax::read_excel(path_for_fixture("fixture-single-sheet-with-types.xlsx"))
            .context("could not read excel file")?;
...
fn test_multiple_sheets() -> Result<()> {
    let mut excel_reader = kyrax::read_excel(path_for_fixture("fixture-multi-sheet.xlsx"))
        .context("could not read excel file")?;
...
fn test_sheet_with_header_row_diff_from_zero() -> Result<()> {
    let mut excel_reader =
        kyrax::read_excel(path_for_fixture("fixture-changing-header-location.xlsx"))
            .context("could not read excel file")?;
...
fn test_header_row_and_skip_rows(
    ...
    #[case] expected: Vec<KyraxColumn>,
) -> Result<()> {
    let mut excel_reader = kyrax::read_excel(path_for_fixture("no-header.xlsx"))
        .context("could not read excel file")?;
