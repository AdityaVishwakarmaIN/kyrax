...
| 3 | **0 unclassified array-write-path rows** | matrix_v2: 20 `CONFIRMED-HYDRATION` rows documented as known deferred write-path spill items (Phase 2); 0 unhandled rows |
...
## Final status (2026-08-23 update)

- 488/488 public standard names are registered and unique.
- `matrix_v2.csv`: 511 rows (491 PASS, 20 HYDRATION write-path deferred, 0 `FAIL`).
- Rust: all integration tests and unit tests green; Lane H: 4 passed.
...
- Evidence: `formula-validation/reports/FINAL_REPORT_v2.md` and `FORMULA_GREEN_PLAN_V5.txt`.
